...
for (var fileExtension in prismKey) {
	...
	(function(fileExtension) {
		$.get(url).done(function(res) {
  		...
  		var code = $('<code class="language-' + prismKey[fileExtension] + '">').text(Base64.decode(res.content));

  		$('.' + fileExtension).append(code)
		$('.language-' + prismKey[fileExtension]).text(Base64.decode(res.content));

		// // need to re-run prism for dynamically generated content
		Prism.highlightAll();
	});
	})(fileExtension);
};

// every tab but first is inactive at start
